...
export default function Section1({ dataHome }) {
  ...
  const headerText =
    language === "fa" ? dataHome?.header_text_farsi : dataHome?.header_text;
...
const startText =
  language === "fa"
    ? toPersianDigits(formatTime(dataHome?.start?.replace("AM", "صبح")))
    : formatTime(dataHome.start);

const endText =
  language === "fa"
    ? toPersianDigits(formatTime(dataHome?.end?.replace("PM", "شب")))
    : formatTime(dataHome.end);
